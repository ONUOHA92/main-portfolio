...
function Navbar(props) {
    ...
    return (
        <div className='flex justify-between items-center w-full h-20 px-4 text-white bg-black fixed'>
            <h1 className='text-5xl font-signature ml-2'>Onuoha</h1>
            <ul className='hidden md:flex'>
                {links.map(({ id, link }) => (
                    <li key={id} className='px-4 cursor-pointer capitalize font-medium  text-gray-500 hover:scal-105 duration-200'>
